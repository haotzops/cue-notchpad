...
- 在“设置 → 编辑器”中可启用 DeepSeek 行间补全：保存 API Key 到 macOS 钥匙串后，Cue 会通过 DeepSeek 官方 `GET /models` 接口获取可选模型；必须从返回的列表中自行选择模型；选择后，光标附近的 prompt 文本会发送给 DeepSeek FIM 生成半透明建议。按 `Tab` 接受建议，按 `Esc` 先关闭建议。该功能默认关闭，候选在接受前不会写入或提交。
- `⌘ ,`：打开设置窗口
